...
class AntTransformerAdapter(BaseModelAdapter):
    """
    Full Ant Transformer with both history lookback (cross-layer attention) and soft gating.
    """
    ...
    def _init_model(self, input_dim: int):
        ...
        # Enable both cross-layer attention and soft gating
        real_model = self.model.module if hasattr(self.model, 'module') else self.model
        real_model.encoder.set_ablation_flags(use_cross_layer=True, use_soft_gating=True)
    ...
    def fit(self, train_df: pd.DataFrame, val_df: pd.DataFrame, features: List[str], target_col: str, seq_len: int = 6):
        ...
        for epoch in range(1, self.config.epochs + 1):
            ...
            for batch in tqdm(train_loader, desc=f"  Epoch {epoch}/{self.config.epochs}", leave=False):
                ...
                optimizer.zero_grad()
                logits, _, all_gates = self.model(x, enable_pruning=True)

                task_loss = criterion(logits.squeeze(-1), y)
                gate_loss = torch.stack([g.abs().mean() for g in all_gates]).mean()
                loss = task_loss + self.config.gate_lambda * gate_loss
