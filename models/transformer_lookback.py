...
class LookbackTransformerAdapter(BaseModelAdapter):
    """
    Transformer with history lookback (cross-layer attention) but NO soft gating.
    """
    ...
    def _init_model(self, input_dim: int):
        ...
        # Enable cross-layer attention, disable soft gating
        real_model = self.model.module if hasattr(self.model, 'module') else self.model
        real_model.encoder.set_ablation_flags(use_cross_layer=True, use_soft_gating=False)
